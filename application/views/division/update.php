...
<?php $this->load->view('components/banner'); ?>
	...
	<?=form_open('division/updateJd/' .  $id . "/" . $divisionAndSub);?>
				...
				<div class="row">
					<div class="col-12 col-sm-6">
									...
									<select class="form-control filter-division filter-select" name="division" id="division" required>
										<?php foreach ($divisions as $key => $value) { ?>
											<?php if (strtolower($value) == strtolower($divisionAndSub)) { ?>
												<option value="<?=strtolower($value);?>" selected><?=$value . " " . $divisionsLabel[strtolower($value)];?></option>
											<?php } else { ?>
												<option value="<?=strtolower($value);?>" ><?=$value . " " . $divisionsLabel[strtolower($value)];?></option>
											<?php } ?>
										<?php } ?>
									</select>
								</div>
						...
						<div class="form-group">
							...
							<input type="text" name="internal_job_title" id="internal_job_title" class="form-control" value="<?=$jdData[0]->internal_job_title;?>"  maxlength="256" required/>
						</div>
					</div>
					<div class="col-12 col-sm-6">
						<div class="form-group">
